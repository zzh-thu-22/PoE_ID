    ...
    for d in dataset:
        ...
        for i in range(0, len(data)):
            ...
            if len(index) == len(options) - 1:
                answer = option_id[np.argmin(log_prob)]
            else:
                new_options = []
                for j in range(len(options)):
                    if j not in index:
                        new_options.append(options[j])
                new_prompt = create_user_prompt(question, new_options)
            ...
            for j in range(0, length-1):
                ...
                new_options.pop(index)
              
                prompt = create_prompt(question, new_options)
                prob = get_token_pr(prompt, len(new_options))
                prob = calculate_prob(all_prior, prob)

            answer_index = np.argmin(prob)
